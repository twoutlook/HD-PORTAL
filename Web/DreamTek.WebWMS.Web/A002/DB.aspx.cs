...
ON T1.cinvcode = T2.PART_FULL WHERE CELL = '{0}'"
    ...
    public string GetHtmlTableWhRec_ROUTINE_NAME(string strSQL)
    {
        // string htmlStr = "";
        StringBuilder sb = new StringBuilder("<table class='gridtable'>");
        string dbConnection = System.Configuration.ConfigurationManager.ConnectionStrings["connstring"].ConnectionString;
        ...
        sb.Append(GetHTMLHeaderWhRec(reader));
        sb.Append(GetHTMLDataWhRec_ROUTINE_NAME(reader));
        sb.Append("</table>");
    ...
    private string GetHTMLDataWhRec_ROUTINE_NAME(SqlDataReader reader)
    {
        StringBuilder sb = new StringBuilder("");
        int k = 0;
        while (reader.Read())
        {
            k++;
            sb.Append(ReadSingleRowV2WhRec_ROUTINE_NAME((IDataRecord)reader, k));
        }
        return sb.ToString();
        //string Id = reader.GetString(0);
        //string OwnerCode = reader.GetString(1);
    ...
    private static string ReadSingleRowV2WhRec_ROUTINE_NAME(IDataRecord record, int rec)
    {
        ...
        for (int i = 0; i < record.FieldCount; i++)
        {
            if (record.GetName(i)== "ROUTINE_NAME")
            {
                sb.AppendFormat("<td><a href='DB2.aspx?name={0}'>{0}</a></td>", record[i]);
            }
            else
            {
                sb.AppendFormat("<td>{0}</td>", record[i]);
